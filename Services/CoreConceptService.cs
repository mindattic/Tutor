...
public class CoreConceptService
{
    ...
    public async Task LoadForUserAsync(string userId)
    {
        ...
        try
        {
            var json = await SecureStorage.GetAsync(ConceptsKeyPrefix + currentUserId);
    ...
    private async Task SaveForUserAsync()
    {
        try
        {
            ...
            await SecureStorage.SetAsync(ConceptsKeyPrefix + currentUserId, json);
        }
        catch { }
    }
    ...
    {
        _concepts.Clear();
        try { SecureStorage.Remove(ConceptsKeyPrefix + currentUserId); } catch { }
        OnConceptsChanged?.Invoke();
