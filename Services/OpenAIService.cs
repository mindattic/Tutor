...
public sealed class OpenAIService : IDisposable
{
    ...
    public async Task<ChatReply> AskAsync(string question, CancellationToken ct = default)
    {
        ...
        var messages = conversationHistory
            .Select(m => new Chat.ChatMessage(m.Role, m.Text, m.Text));
        ...
        CancellationToken ct = default)
    {
        ...
        var messages = new[]
        {
            new Chat.ChatMessage("user", question, question)
        };
...
public sealed class OpenAIOptions
{
    ...
    {
        Microsoft.Maui.Storage.SecureStorage.Remove("OPENAI_API_KEY");
    }
}
